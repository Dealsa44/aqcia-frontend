...
export class MarketDetailsComponent {
  ...
  loadMarketData() {
    ...
    const marketProducts = productsMocks.products.filter(product => 
      product.prices.some(price => price.market.toLowerCase() === this.marketId.toLowerCase())
    );
    ...
    const categoryIds = [...new Set(marketProducts.map(product => product.category))];
    ...
    const prices = marketProducts.flatMap(product => 
      product.prices
        .filter(price => price.market.toLowerCase() === this.marketId.toLowerCase())
    ...
    if (prices.length > 0) {
      ...
      this.discountCount = marketProducts.filter(product => 
        product.prices.some(price => 
          price.market.toLowerCase() === this.marketId.toLowerCase() && price.discount > 0
        )
      ).length;
    }
  }
